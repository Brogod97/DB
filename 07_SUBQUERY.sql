...
JOIN JOB USING(JOB_CODE)
WHERE EMP_ID IN (SELECT DISTINCT MANAGER_ID FROM EMPLOYEE WHERE MANAGER_ID IS NOT NULL)
UNION -- 연결할 RESULT SET 사이에 UNION 키워드만 삽입하면 됨
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, '사원'
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
...
-- > ANY : 과장급여 3개 중, 최소금액인 220 보다 더 큰 SALARY
-- < ANY : 과장급여 3개 중, 최대금액인 376 보다 더 작은 SALARY
...
SELECT DEPT_ID
FROM DEPARTMENT
WHERE LOCATION_ID = (SELECT LOCAL_CODE FROM LOCATION WHERE NATIONAL_CODE = 'KO'); -- D1, D2, D3, D4, D9
...
-- 메인 쿼리의 테이블값이 변경되면 서브쿼리의 결과값도 바뀌게 되는 구조임!
...
-- EPMLOYEE(23행) -> 23개의 서브쿼리
						
-- 직급별 급여 평균보다 급여를 많이 받는 직원의 
-- 이름, 직급코드, 급여 조회
						
-- MAIN -> SUB -> MAIN 순서로 처리
-- 메인 쿼리 수에 따라 서브 쿼리의 수가 달라짐 (메인쿼리가 서브쿼리에 영향) 
