...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE -- DEPT_CODE, DEPT_ID는 같은 값이라서 동일 컬럼 두개 생김
FROM EMPLOYEE
...
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE); -- EMPLOYEE 테이블 + DEPARTMENT 테이블 + LOCATION 테이블
...
SELECT EMP_NAME 사원명, BONUS 보너스포인트, NVL(DEPT_TITLE, '-') 부서명, NVL(LOCAL_NAME, '-') 근무지역명
FROM EMPLOYEE
