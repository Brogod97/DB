...
-- 객체를 만들고(CREATE), 바꾸고(ATLER), 삭제(DROP)하는 데이터 정의 언어

-- ALTER(바꾸다, 수정하다, 변조하다)

-- 테이블에서 수정할 수 있는 것
-- 1) 제약 조건(추가/삭제) -- 기존의 것을 변경하는 것은 불가능
-- 2) 컬럼(추가/수정/삭제)
...
--> 수정은 별도 존재하지 않기 떄문에 삭제 후 추가를 이용해서 수정
...
-- DEPT_TITLE 컬럼을 NOT NULL 제약조건 설정으로 수정
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; 

-- DEPT_TITLE 컬럼을 NOT NULL 제약조건 해제
ALTER TABLE DEPT_COPY
...
SELECT * FROM DEPT_COPY; -- 삭제 성공 확인
...
-- DROP TALBE 테이블명 [CASCADE CONSTARINTS];
...
-- 3) DROP TALBE 삭제 옵션 CASCADE CONSTRAINTS 사용
...
SELECT * FROM TB2; -- TB2_COLCOL로 됨 (즉, ALTER 구문에서 COMMIT이 자동 수행 됨)
