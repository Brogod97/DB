...
-- 2. 휴학중인 학생들의 이름과 주민번호를 나이가 적은 수서로 화면에 출력하시오.
...
SELECT STUDENT_NO, TO_CHAR(POINT, '9.00') AS "POINT"
FROM TB_GRADE tg 
...
-- 11. 학번이 A313047인 학생이 학교에 나오지 있지 않다.
-- 지도 교수에게 내용을 전달하기 위한 학과 이름, 학생 이름과 지도 교수 이름이 필요하다.
-- 이 때 사용할 SQL문을 작성하시오.
-- 단, 출력 헤더에는 "학과이름", "학생이름", "지도교수이름"으로 출력되도록 한다.
...
-- 학생이름과 지도교수 이름을 찾고 지도 교수가 없는 학생일 경우 "지도교수 미지정"으로 표시하도록 하는 SQL 문을 작성하시오
-- 단, 출력헤더는 "학생이름", "지도교수"로 표시하며 고학번 학생이 먼저 표시되로록 한다.
...
-- 18. 국어국문학과에서 총 평점이 가장 높은 학생의 이름과 학번을 표시하는 SQL문을 작성하시오 -- 다시 !
...
-- 19. 춘 기술대학교의 '환경조성학과'가 속한 같은 계열 학과들의 학과 별 전공과목 평점을 파악하기 위한 적절한 SQL문을 찾아내시오. -- 다시 !
-- 단, 출력헤더는 "계열 학과명", "전공평점"으로 표시되도록 하고, 평점은 소수점 한 자리까지만 반올림하여 표시되도록 한다.
SELECT AS DEPARTMENT_NAME AS "계열 학과명", ROUND(AVG(POINT), 1) AS "전공평점"
FROM TB_DEPARTMENT
