...
-- 테이블에 값을 삽입하거나(INSERT) , 수정(UDPATE), 삭제(DELETE) 하는 구문

-- 주의 : 혼자 COMMIT, ROLLBACK 하지 마세요
...
SELECT * FROM EMPLOYEE2; -- 복제 테이블 확인 (EMPLOYEE2)
SELECT * FROM DEPARTMENT2; -- 복제 테이블 확인 (DEPARTMENT2)
...
ROLLBACK; -- 이전 시점으로 돌아감. 장채현 추가 되어 있지 않음

DELETE FROM EMPLOYEE2
WHERE EMP_ID = '900'; -- 조건에 해당하는 값 전체 삭제

COMMIT; -- COMMIT을 하고 나면 ROLLBACK을 해도 돌아갈 수 없음
		-- COMMIT은 확실한 시점에 실행
		-- COMMIT을 되돌리는 방법도 있으나 복잡하니 신중하게 커밋 여부 결정
...
VALUES('900', '장채현', '901123-2345678', 'test@test.com', 
		'01012341234', 'D1', 'J7', 'S3', 4300000);
...
-- [WHERE 컬럼명 비교연산자 비교값];
--> WHERE 조건 중요!
-- UPDATE할 컬럼과 조건을 WHERE 절에서 정하기 때문
...
SELECT * FROM EMPLOYEE2; --> 24개 행 조회
